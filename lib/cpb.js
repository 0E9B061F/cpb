...
class NSTGlob {
  ...
  match(nstu) {
    return !!nstu.unshort.match(this.re)
  }
}

class NSTReserved {
  constructor(glob, type, purpose) {
    this.glob = glob
    this.type = type
    this.purpose = purpose
  }
  match(nstu) { return this.glob.match(nstu) }
}
...
class NSTU {
  ...
  static reserved = [
    new NSTReserved(new NSTGlob('.*:menu'), 'directory', 'create a menu for this namespace'),
    new NSTReserved(new NSTGlob('~.*:bookmarks'), 'directory', "store this user's bookmarks"),
    new NSTReserved(new NSTGlob('~.*:config'), 'object', "store this user's configuration"),
    new NSTReserved(new NSTGlob(':bookmarks'), 'directory', 'store system bookmarks'),
  ]
  static mkconf =(...confs)=> {
  ...
  checkreserves() {
    let out = false
    let r
    for (let i = 0, m = NSTU.reserved.length; i < m; i++) {
      r = NSTU.reserved[i]
      if (r.match(this)) {
        out = r
        break
  ...
  normalize() {
    ...
    if (this.valid) {
      ...
      if (this.tail && this.tail.length) {
        base = `${base}/${this.tail.join('/')}`
        if (unshort) unshort = `${unshort}/${this.tail.join('/')}`
      }
      base = base.replace(/ /g, '_')
      if (unshort) unshort = unshort.replace(/ /g, '_')
      full = base
      unshortfull = unshort
      if (this.optstr) {
        full = `${full}${this.optstr}`
        if (unshortfull) unshortfull = `${unshortfull}${this.optstr}`
      }
      if (this.hash) {
        full = `${full}#${this.hash}`
        if (unshortfull) unshortfull = `${unshortfull}#${this.hash}`
      }
    }
    this.base = base
    this.normal = full
    this.unshort = unshort || base
    this.unshortfull = unshortfull || full
