...
class NSTU {
  ...
  constructor(...confs) {
    ...
    if (this.title == '@') {
      const d = new Date()
  		this.title = date.format(d, 'YYYY-MM-DD-SSS')
    }
    this.tail = this.conf.tail

    if (this.title && !this.namespace) this.namespace = this.from
    ...
    this.nospace = !this.uuid && !this.namespace
    this.index = !this.uuid && !this.title
    this.userpage = !this.uuid && this.userspace && this.index
    this.contentspace = !this.uuid && !this.index && !this.nospace
